package com.example.optimization_algorithm_backend.algorithm.algorithm2;
...
public class Algorithm2 {  // 双模式局部搜索算法
    Map<String, ArrayList<String>> pathList;
    ArrayList<String> sortNodes;
    ArrayList<String> visited;  // 记录已经访问过的节点
    ...
    public void adjustPath(ProcessMap map){
        ...
        for(int i=0;i<node1.size();i++){
            int j = i;
            map.getProcessPaths().removeIf(p -> p.getStartNodeID().equals(node0.get(j)) && p.getEndNodeID().equals(node2.get(j)));
            map.getProcessPaths().removeIf(p -> p.getStartNodeID().equals(node1.get(j)) && p.getEndNodeID().equals(node2.get(j)));
            map.getProcessPaths().add(new ProcessPath(utils.getRandomString(5), node1.get(j), node2.get(j)));
        }

    }
    public ArrayList<String> GetNodesCanBeUsed(ProcessMap map, String start, String end){
        ArrayList<String> res = new ArrayList<>();
        ArrayList<String> nodes = new ArrayList<>();
        for(ProcessPath path: map.getProcessPaths()){
            if(path.getStartNodeID().equals(start)&& !path.getEndNodeID().equals(end)){
...
//        System.out.println(nodes);
        ...
        for(String node: nodes){
            if(!HasPath(map.getProcessPaths(), node, end)){
                res.add(node);
            }
        }
        return res;
    }
    public boolean HasPath(LinkedList<ProcessPath> paths, String from, String to){
        if(from.equals(to)) return true;
        boolean flag = false;
        for(ProcessPath path: paths){
            if(path.getStartNodeID().equals(from)){
                flag = flag || HasPath(paths, path.getEndNodeID(), to);
            }
        }
        return flag;
    }
    public ProcessMap IterativeOptimize(ProcessMap oldMap, int x1){
        ...
        String end;
        int count = 0;
        while(this.visited.size()<this.sortNodes.size()){
            if(x1==2){
                int size = this.sortNodes.size() / 10;
                if(count%(size*2) <= size){
                    end = selectNode(oldMap, "greedy");
                } else {
                    end = selectNode(oldMap, "random");
                }
            } else {
                if(count%10 <= 5)
                    end = selectNode(oldMap, "random");
            ...
            count++;
            this.visited.add(end);
            nodesWithConstraint = getNodeWithConstraint(end, newConstraintConditions, this.sortNodes);
                ...
                for(String start : nodesWithConstraint){
                    ...
                    if (lastTwoNodes.isEmpty()){
                        if(!this.pathList.get(start).contains(end)){
                            ...
                            String finalEnd1 = end;
                            newProcessPaths.removeIf(p -> p.getStartNodeID().equals(start) && p.getEndNodeID().equals(finalEnd1));
                        ...
                        for(String node: lastTwoNodes){
                            ...
                            if(!start.equals(node) && !this.pathList.get(start).contains(end)){
                                this.pathList.get(start).add(end);
                                String finalEnd = end;
                                newProcessPaths.removeIf(p -> p.getStartNodeID().equals(start) && p.getEndNodeID().equals(finalEnd));
                                newProcessPaths.add(new ProcessPath(utils.getRandomString(5), start, end));
                            }
                            if(!start.equals(node) && !flag){
                                this.pathList.get(node).remove(end);
                                String finalEnd = end;
                                newProcessPaths.removeIf(p -> p.getStartNodeID().equals(node) && p.getEndNodeID().equals(finalEnd));
    ...
    public String selectNode(ProcessMap oldMap, String type){
        ...
        if(type.equals("greedy")){
            for(String node: this.sortNodes){
                if(!this.visited.contains(node)){
                    return node;
                }
            }
        } else if(type.equals("random")){
            nodeList.removeIf(p -> this.visited.contains(p));
            Random random = new Random();
            int max = nodeList.size()-1, min = 0;
